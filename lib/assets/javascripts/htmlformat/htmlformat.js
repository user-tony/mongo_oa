  ...
  function Parser() {
    ...
    this.get_content = function () { //function to capture regular content between tags
      ...
      while (this.input.charAt(this.pos) !== '<') {
        ...
        char_data = this.input.charAt(this.pos);
        this.pos++;
        this.line_char_count++;
        
        
        if (this.Utils.in_array(char_data, this.Utils.whitespace)) {
          if (content.length) {
            space = true;
          }
          this.line_char_count--;
        ...
        content.push(char_data); //letter at-a-time (or string) inserted to an array
      }
    ...
    this.get_tag = function () { //function to get a full tag and parse its type
      ...
      do {
        ...
        if (this.Utils.in_array(char_data, this.Utils.whitespace)) { //don't want to insert unnecessary space
          space = true;
          this.line_char_count--;
          continue;
        }
        ...
        if (char_data === "'" || char_data === '"') {
          if (!content[1] || content[1] !== '!') { //if we're in a comment strings don't get treated specially
            char_data += this.get_unformatted(char_data);
            space = true;
          }
        }
        
        if (char_data === '=') { //no space before =
          space = false;
        }
        
        if (content.length && content[content.length-1] !== '=' && char_data !== '>'
        ...
        content.push(char_data); //inserts character at-a-time (or string)
      } while (char_data !== '>');
