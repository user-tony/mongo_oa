(function() {
    ...
    $.jQTouch = function(options) {
        ...
        function doNavigation(fromPage, toPage, animation, goingBack) {
			...
			if (window.history.pushState) window.history.pushState({}, '', $currentPage.attr('href'));
		...
		function insertPages(nodes, animation, appending) {
            ...
            $(div).children().each(function(index, node) {
				...
				if (!appending && index == 0) { var page = $body.children('.current:not(.out)'); page.children().remove(); page.append($node.children()); $body.trigger('pageInserted', {page: page}); return; }
                $body.append($node);
        ...
        function showPageByHref(href, options) {
            ...
            if (href !== '#') {
				insertPages("<div href='"+href+"'><div class='toolbar'><h1>"+(settings.$referrer ? settings.$referrer.attr('title') || settings.$referrer.text() : "Loading...")+"</h1></div></div>", settings.animation, true);
				href = href.replace(/^#!/, '')
                $.ajax({
                    url: href,
                    data: settings.data,
                    ...
                    success: function (data) {
						data = data.replace(/^[\s\S]*<!--\s?jqt_begin\s?-->/m, '').replace(/<!--\s?jqt_end\s?-->[\s\S]*$/m, '')
        ...
        function tapHandler(e) {
			...
			if ($el.attr('href')[0] != '#') $el.attr('href', '#!' + $el.attr('href'));
			if (hash.match(/^#!/)) hash = '';
