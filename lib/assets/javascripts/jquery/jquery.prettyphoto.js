...
(function($) {
	$.fn.prettyPhoto = function(settings) {
		...
		settings = jQuery.extend({
			...
			padding: 40, /* padding for each side of the pictures */
		...
		function open(el) {
			...
			// Find out if the pictures is part of a set
			theRel = $caller.attr('rel');
			galleryRegExp = /\[(?:.*)\]/;
			theGallery = galleryRegExp.exec(theRel);
		
			// Calculate the number of items in the set, and the position of the clicked pictures.
			isSet = false;
			setCount = 0;
			...
			// Position the pictures in the center of the viewing area
			_centerPicture();
		...
		function _checkPosition(){
			...
			// Change the current pictures text
			$pp_pic_holder.find('p.currentTextHolder').text(setPosition + settings.counter_separator_label + setCount);
		...
		function _buildOverlay(){
			...
			// Basic HTML for the pictures holder
